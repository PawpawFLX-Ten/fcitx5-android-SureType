...
public:
    ...
    void updateCandidatesBulk() {
        ...
        std::vector<CandidateEntity> pagedCandidates;
        int size = 0;
        ...
        if (list) {
            ...
            if (bulk) {
                size = bulk->totalSize();
                // Keep the visible row at 16; expose up to 128 structured candidates for PinyinBar.
                const int visibleLimit = 16;
                const int pinyinLimit = size < 0 ? 128 : std::min(size, 128);
                pagedCandidates.reserve(pinyinLimit);
                for (int i = 0; i < pinyinLimit; i++) {
                    try {
                        auto &candidate = bulk->candidateFromAll(i);
                        if (i < visibleLimit) {
                            candidates.emplace_back(filterCandidate(candidate));
                        }
                        pagedCandidates.emplace_back(
                                "",
                                filterString(candidate.text()),
                                filterString(candidate.comment())
                        );
        ...
        if (pagedCandidates.empty()) {
            frontend_->updatePagedCandidate(PagedCandidateEntity::Empty);
        } else {
            frontend_->updatePagedCandidate(PagedCandidateEntity(
                    pagedCandidates,
                    -1,
                    list->layoutHint(),
                    false,
