...
import org.fcitx.fcitx5.android.core.FCITX_CANDIDATE_RESTORE_LIMIT
import org.fcitx.fcitx5.android.core.FcitxEvent
import org.fcitx.fcitx5.android.core.shouldRestoreCachedCandidates
import org.fcitx.fcitx5.android.core.stripTrailingParenReadings
...
import org.fcitx.fcitx5.android.input.candidates.pinyin.PinyinBarComponent
import org.fcitx.fcitx5.android.input.candidates.pinyin.PinyinCandidateController
...
class InputView(
    ...
    private val pinyinBar = PinyinBarComponent()
    private val pinyinCandidateController = PinyinCandidateController()
    ...
    private fun updateKeyboardSize() {
        ...
        pinyinBar.view.setPadding(sidePadding, 0, sidePadding, 0)
    ...
    override fun onStartHandleFcitxEvent() {
        ...
        buildList {
            add(FcitxEvent.InputPanelEvent(inputPanelData))
            if (clientPreedit.isNotEmpty()) {
                add(FcitxEvent.ClientPreeditEvent(clientPreedit.stripTrailingParenReadings()))
            }
            add(FcitxEvent.IMChangeEvent(inputMethodEntry))
            add(
    ...
    /**
     * Runs after [FcitxInputMethodService.postFcitxJob] focus work (see [onStartInput] ordering).
     * Tries live [getCandidates] first, then falls back to the sticky restore cache.
     */
    private fun scheduleRestoreCandidateUi() {
        service.postFcitxJob {
            val lines = getCandidates(0, FCITX_CANDIDATE_RESTORE_LIMIT)
            val hasCache = shouldRestoreCachedCandidates()
            if (lines.isEmpty() && !hasCache) return@postFcitxJob
            if (lines.isNotEmpty()) {
                val total = if (lines.size < FCITX_CANDIDATE_RESTORE_LIMIT) {
                    lines.size
                } else {
                    lines.size + 1
                }
                handleFcitxEvent(
                    FcitxEvent.CandidateListEvent(
                        FcitxEvent.CandidateListEvent.Data(total = total, candidates = lines)
                    )
                )
            } else {
                candidateListCached?.let {
                    handleFcitxEvent(FcitxEvent.CandidateListEvent(it))
                }
            }
            pagedCandidateCached?.let {
                handleFcitxEvent(FcitxEvent.PagedCandidateEvent(it))
            }
        }
    }
    ...
    override fun handleFcitxEvent(it: FcitxEvent<*>) {
        when (it) {
            ...
            is FcitxEvent.ClientPreeditEvent -> {
                val clientPreedit = it.data.stripTrailingParenReadings()
                preeditEmptyState.updatePreeditEmptyState(clientPreedit = clientPreedit)
                broadcaster.onClientPreeditUpdate(clientPreedit)
            }
