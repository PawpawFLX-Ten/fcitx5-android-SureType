    ...
    companion object {
        ...
        val Layout: List<List<KeyDef>> = listOf(
            ...
            listOf(
                ...
                SuretypeKey("L", "L", swipeUp = "0", swipeDown = ")", swipeRight = ";", popup = arrayOf(
            ...
            listOf(
                ...
                SuretypeKey("M", "M", percentWidth = 0.175f, swipeUp = "/", swipeDown = "\\", swipeRight = "?", popup = arrayOf(
        ...
        when (action) {
            is KeyAction.DirectKeyAction -> when (source) {
                KeyActionListener.Source.Keyboard -> {
                    val act = transformAlphabet(action.act)
                    transformed = when (capsState) {
                        CapsState.None -> KeyAction.FcitxKeyAction(act.lowercase())
                        CapsState.Once -> {
                            switchCapsState()
            ...
            is KeyAction.FcitxKeyAction -> when (source) {
                KeyActionListener.Source.Keyboard -> {
                    val act = transformAlphabet(action.act)
                    transformed = when (capsState) {
                        CapsState.None -> action.copy(act = act.lowercase())
                        CapsState.Once -> {
                            switchCapsState()
                            action.copy(
                                act = act.uppercase(),
