...
open class PreeditUi(
    ...
    fun update(inputPanel: FcitxEvent.InputPanelEvent.Data) {
        val data = inputPanel.sanitizedForDisplay()
        val activeBkg = theme.genericActiveBackgroundColor
        val upString: SpannedString
        val upCursor: Int
        if (data.auxUp.isEmpty()) {
            upString = data.preedit.toSpannedString(activeBkg)
            upCursor = data.preedit.cursor
        } else {
            upString = buildSpannedString {
                append(data.auxUp.toSpannedString(activeBkg))
                append(data.preedit.toSpannedString(activeBkg))
            }
            upCursor = data.preedit.cursor.let {
                if (it < 0) it
                else data.auxUp.length + it
            }
        }
        val downString = data.auxDown.toSpannedString(activeBkg)
...
private fun FcitxEvent.InputPanelEvent.Data.sanitizedForDisplay(): FcitxEvent.InputPanelEvent.Data =
    copy(
        preedit = preedit.stripTrailingParenReadings(),
        auxUp = auxUp.stripTrailingParenReadings(),
        auxDown = auxDown.stripTrailingParenReadings(),
    )
