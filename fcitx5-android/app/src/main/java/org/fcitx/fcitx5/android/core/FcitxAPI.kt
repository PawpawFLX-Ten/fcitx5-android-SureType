...
interface FcitxAPI {
    ...
    /** Last non-empty bulk candidate list (virtual keyboard strip). */
    val candidateListCached: FcitxEvent.CandidateListEvent.Data?
        get() = null

    /** Last non-empty paged candidates (structured text + comment for PinyinBar). */
    val pagedCandidateCached: FcitxEvent.PagedCandidateEvent.Data?
        get() = null

    /** Drops sticky candidate snapshots (commit, reset, or user cleared composition while typing). */
    fun clearCandidateRestoreSnapshot()
