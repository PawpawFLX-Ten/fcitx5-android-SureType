...
private val trailingParenReadingBlock = Regex(
    """(?:\s*[（(［\[](?=[^）\]］\n\r]*[A-Za-z0-9])[^（(\n\r]{1,160}[）)\]］])$"""
)

/**
 * Strips trailing Rime reading suffixes `(pinyin)`, `（pinyin）`, `［pinyin］` from IME display buffers.
 */
fun stripParenReadingTail(text: String): String {
    var t = text
    while (t.isNotEmpty()) {
        val m = trailingParenReadingBlock.find(t) ?: break
        t = t.removeRange(m.range)
    }
    return t
}

/** Reading inside the last trailing paren/bracket block, or null if none. */
fun extractTrailingParenReading(text: String): String? {
    val m = trailingParenReadingBlock.find(text) ?: return null
    return m.value.trim()
        .trimStart('（', '(', '［', '[')
        .trimEnd('）', ')', '］', ']')
        .trim()
...
/**
 * Per-string-segment stripping so [FormattedText.flags] spans stay aligned; adjusts [cursor].
 */
fun FormattedText.stripTrailingParenReadings(): FormattedText {
    if (strings.isEmpty()) return this
    val out = Array(strings.size) { stripParenReadingTail(strings[it]) }
    var nc = cursor
    var segStartOld = 0
    if (nc >= 0) {
        for (i in strings.indices) {
            val oldSeg = strings[i]
            val newSeg = out[i]
            val rm = oldSeg.length - newSeg.length
            if (rm > 0) {
                val segEndOld = segStartOld + oldSeg.length
                val rmStartOld = segEndOld - rm
                when {
                    nc >= segEndOld -> nc -= rm
                    nc > rmStartOld -> nc = rmStartOld
                }
            }
            segStartOld += oldSeg.length
        }
    }
    val newLen = out.sumOf { it.length }
    val clamped = when {
        nc < 0 -> nc
        else -> nc.coerceIn(0, newLen.coerceAtLeast(0))
    }
    return FormattedText(out, flags.copyOf(), clamped)
}
