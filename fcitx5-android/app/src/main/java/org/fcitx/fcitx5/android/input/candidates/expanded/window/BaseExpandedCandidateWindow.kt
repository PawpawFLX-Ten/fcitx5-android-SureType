...
abstract class BaseExpandedCandidateWindow<T : BaseExpandedCandidateWindow<T>> :
    ...
    private fun newCandidatesPager() = Pager(
        config = PagingConfig(
            pageSize = 48,
    ...
    private var candidatesPager = newCandidatesPager()
    private var candidatesSubmitJob: Job? = null

    private fun restartCandidatesPager() {
        candidatesSubmitJob?.cancel()
        candidatesPager = newCandidatesPager()
    ...
    override fun onAttached() {
        PinyinCandidateFilter.onFilterChanged = { restartCandidatesPager() }
        ...
        restartCandidatesPager()
    }

    override fun onCandidateUpdate(data: CandidateListEvent.Data) {
        // Refresh in place; full pager restart is reserved for chip filter changes.
        adapter.refresh()
    }
    ...
    override fun onDetached() {
        ...
        candidatesSubmitJob?.cancel()
        PinyinCandidateFilter.onFilterChanged = null
        offsetJob?.cancel()
