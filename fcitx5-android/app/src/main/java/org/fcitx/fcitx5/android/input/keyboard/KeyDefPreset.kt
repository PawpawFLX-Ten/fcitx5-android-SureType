...
class SuretypeKey(
    ...
    Appearance.HorizontalSuretype(
        ...
        swipeLeftHint = swipeLeft,
        swipeRightHint = swipeRight,
        textSize = 22f,
        percentWidth = percentWidth,
        variant = variant
    ),
    buildSet {
        ...
        add(Behavior.SwipeLeft(KeyAction.DirectKeyAction(swipeLeft ?: primary)))
        add(Behavior.SwipeRight(KeyAction.DirectKeyAction(swipeRight ?: secondary)))
    ...
    popup ?: arrayOf(
        Popup.DirectionalPreview(primary, swipeLeft ?: primary, swipeRight ?: secondary, swipeUp, swipeDown),
...
class T9DigitKey(
    ...
    setOfNotNull(
        Behavior.Press(KeyAction.FcitxKeyAction(digit)),
        letters.takeIf { it.isNotEmpty() }?.let {
            Behavior.Swipe(KeyAction.FcitxKeyAction(it.first().toString()))
        },
        swipeLeft?.let { Behavior.SwipeLeft(KeyAction.FcitxKeyAction(it)) },
        swipeRight?.let { Behavior.SwipeRight(KeyAction.FcitxKeyAction(it)) },
        swipeUp?.let { Behavior.SwipeUp(KeyAction.FcitxKeyAction(it)) },
        swipeDown?.let { Behavior.SwipeDown(KeyAction.FcitxKeyAction(it)) }
    ),
    popup = arrayOf(
        Popup.DirectionalPreview(
            digit,
            swipeLeft ?: digit,
            swipeRight ?: letters.firstOrNull()?.toString() ?: digit,
            swipeUp,
            swipeDown
        )
    )
