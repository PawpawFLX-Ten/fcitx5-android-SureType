...
abstract class BaseKeyboard(
    ...
    private fun createKeyView(def: KeyDef): KeyView {
        return when (def.appearance) {
                ...
                when (it) {
                    ...
                    is KeyDef.Popup.Menu -> {
                        setOnLongClickListener { view ->
                            view as KeyView
                            onPopupAction(PopupAction.ShowMenuAction(view.id, it, view.freshBounds()))
                    ...
                    is KeyDef.Popup.Keyboard -> {
                        setOnLongClickListener { view ->
                            view as KeyView
                            onPopupAction(PopupAction.ShowKeyboardAction(view.id, it, view.freshBounds()))
                            false
                        }
                        val oldOnGestureListener = onGestureListener ?: OnGestureListener.Empty
                        swipeEnabled = true
                            ...
                            if (popupOnKeyPress) {
                                when (event.type) {
                                    GestureType.Down -> onPopupAction(
                                        PopupAction.PreviewAction(view.id, it.content, view.freshBounds())
                                    )
                                    GestureType.Move -> {
                        ...
                        onGestureListener = OnGestureListener { view, event ->
                            ...
                            if (popupOnKeyPress) {
                                when (event.type) {
                                    ...
                                    GestureType.Move -> {
                                        val text = when (resolveKeySwipeDirection(event.totalX, event.totalY)) {
                                            "left" -> it.leftHint
                                            "right" -> it.rightHint
                                            "up" -> it.upHint ?: it.content
                                            "down" -> it.downHint ?: it.content
                                            else -> it.content
                                        }
            ...
            if (dirActions.isNotEmpty()) {
                swipeEnabled = true
                swipeThresholdX = dp(12f)
                swipeThresholdY = dp(12f)
                val oldOnGestureListener = onGestureListener ?: OnGestureListener.Empty
                onGestureListener = OnGestureListener { view, event ->
                    when (event.type) {
                        GestureType.Up -> {
                            if (event.consumed) {
                                ...
                                if (!hasMovement) {
                                    ...
                                    val dir = resolveKeySwipeDirection(event.totalX, event.totalY)
    ...
    /**
     * Builds the space bar label from IME [InputMethodEntry].
     * Handles ASCII mode ("A" → "英"), Latin fallback, and display-name dedup.
     */
    private fun KeyView.freshBounds(): Rect {
        updateBounds()
        return bounds
    }

    protected fun spaceBarLabel(ime: InputMethodEntry): String = buildString {
        val mode = InputModeRegistry.modeForSubModeNameOrSchemaId(ime.subMode.name, ime.schemaId)
        append(mode?.userLabel ?: ime.displayName)
        val subModeText = when {
            ime.subMode.label == "A" -> "英"
            ime.subMode.label.isNotEmpty() -> ime.subMode.label
            ime.subMode.name.contains("Latin", ignoreCase = true) -> "英"
            else -> ime.subMode.name.ifEmpty { null }
        }
        subModeText
            ?.takeIf { it != mode?.userLabel && it != ime.displayName }
            ?.let { append(" ($it)") }
    }
