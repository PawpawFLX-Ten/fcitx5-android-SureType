...
class HorizontalSuretypeKeyView(ctx: Context, theme: Theme, def: KeyDef.Appearance.HorizontalSuretype) :
    ...
    private val swipeLeftHintText = view(::AutoScaleTextView) {
        isClickable = false
        isFocusable = false
        background = null
        visibility = if (def.swipeLeftHint != null) View.VISIBLE else View.GONE
        text = def.swipeLeftHint ?: ""
        setTextSize(TypedValue.COMPLEX_UNIT_DIP, 22f)
        textDirection = View.TEXT_DIRECTION_FIRST_STRONG_LTR
        setTextColor(theme.keyTextColor)
    }

    private val swipeRightHintText = view(::AutoScaleTextView) {
        isClickable = false
        isFocusable = false
        background = null
        visibility = if (def.swipeRightHint != null) View.VISIBLE else View.GONE
        text = def.swipeRightHint ?: ""
        setTextSize(TypedValue.COMPLEX_UNIT_DIP, 22f)
        textDirection = View.TEXT_DIRECTION_FIRST_STRONG_LTR
        setTextColor(theme.keyTextColor)
    }
    ...
    init {
        appearanceView.apply {
            ...
            add(swipeLeftHintText, lParams(wrapContent, wrapContent) {
                topToTop = parentId
                bottomToBottom = parentId
                leftToLeft = parentId
                leftMargin = dp(10)
            })
            add(swipeRightHintText, lParams(wrapContent, wrapContent) {
                topToTop = parentId
                bottomToBottom = parentId
                rightToRight = parentId
                rightMargin = dp(10)
