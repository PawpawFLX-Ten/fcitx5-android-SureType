...
class CandidatesView(
    ...
    override fun handleFcitxEvent(it: FcitxEvent<*>) {
        when (it) {
            ...
            is FcitxEvent.PagedCandidateEvent -> {
                paged = handJumpSanitizedPagedCandidates(it.data)
                updateUi()
            }
            else -> {}
        }
    ...
    /** Physical-keyboard floating candidates: Han-only text, no comment column. */
    private fun handJumpSanitizedPagedCandidates(
        data: FcitxEvent.PagedCandidateEvent.Data
    ): FcitxEvent.PagedCandidateEvent.Data {
        val ime = fcitx.runImmediately { inputMethodEntryCached }
        if (ime.isFcitxPlaceholder(context) || !InputModeRegistry.pinyinGroupingEnabled(ime)) {
            return data
        }
        val candidates = data.candidates.map { candidate ->
            ...
            FcitxEvent.Candidate(
                ...
                text = HandJumpCandidateDisplay.displayText(candidate.text, reading),
                comment = ""
            )
        }.toTypedArray()
        return data.copy(candidates = candidates)
    }
