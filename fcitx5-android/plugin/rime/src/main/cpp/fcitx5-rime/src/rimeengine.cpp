...
std::string RimeEngine::subModeIdentifier(const InputMethodEntry & /*entry*/,
                                          InputContext &ic) {
    if (auto *rimeState = state(&ic)) {
        return rimeState->currentSchema();
    }
    return "";
}
...
void RimeEngine::updateSchemaMenu() {
    ...
    if (api_->get_schema_list(&list)) {
        // Latin Mode menu entry omitted — HandJump uses globe / ascii_mode only.
